...
//            "{\"type\":\"Feature\",\"properties\":{\"mag\":6,\"place\":\"Pacific-Antarctic Ridge\",\"time\":1451986454620,\"updated\":1459202978040,\"tz\":-540,\"url\":\"http://earthquake.usgs.gov/earthquakes/eventpage/us10004bgk\",\"detail\":\"http://earthquake.usgs.gov/fdsnws/event/1/query?eventid=us10004bgk&format=geojson\",\"felt\":0,\"cdi\":1,\"mmi\":0,\"alert\":\"green\",\"status\":\"reviewed\",\"tsunami\":0,\"sig\":554,\"net\":\"us\",\"code\":\"10004bgk\",\"ids\":\",us10004bgk,gcmt20160105093415,\",\"sources\":\",us,gcmt,\",\"types\":\",cap,dyfi,geoserve,losspager,moment-tensor,nearby-cities,origin,phase-data,shakemap,\",\"nst\":null,\"dmin\":30.75,\"rms\":0.67,\"gap\":71,\"magType\":\"mww\",\"type\":\"earthquake\",\"title\":\"M 6.0 - Pacific-Antarctic Ridge\"},\"geometry\":{\"type\":\"Point\",\"coordinates\":[-136.2603,-54.2906,10]},\"id\":\"us10004bgk\"}],\"bbox\":[-153.4051,-54.2906,10,158.5463,59.6363,582.56]}";
    ...
    private static URL createURL(String stringURL){
        ...
        try{
            ...
            Log.e(LOG_TAG , "Error in URL formation", e);
        }
        return url;
    }
    ...
    private static String readFromStream(InputStream inputStream){
        ...
        if (inputStream!=null) {
            ...
            try {
                ...
                Log.e(LOG_TAG,"Error reading line",e);
            }
            while (line != null) {
                output.append(line);
                try {
    ...
    public static List<earthquake> extractEarthquakes(String earthquakeJSON) {
        ...
        try {
            ...
            JSONArray earthqakeArray = baseJSONResponse.getJSONArray("features");
            SimpleDateFormat dateFormatter = new SimpleDateFormat("LLL dd, yyyy");
            SimpleDateFormat timeFormatter = new SimpleDateFormat("h:mm a");
            for(int i=0;i<earthqakeArray.length();i++)
            {
                earthquake quake = new earthquake(earthqakeArray.getJSONObject(i).getJSONObject("properties").getDouble("mag"),
                        earthqakeArray.getJSONObject(i).getJSONObject("properties").getString("place"),
                        dateFormatter.format(new Date(earthqakeArray.getJSONObject(i).getJSONObject("properties").getLong("time"))),
                        timeFormatter.format(new Date(earthqakeArray.getJSONObject(i).getJSONObject("properties").getLong("time"))),
                        earthqakeArray.getJSONObject(i).getJSONObject("properties").getString("url"));
